...
fun SadChive(
    ...
    isError: Boolean,
    navController: NavController// The droop animation only plays if this is true (e.g., when an error state occurs)
) {
    ...
    Column(modifier = Modifier.fillMaxWidth().fillMaxHeight().padding(50.dp),) {
        ...
        Canvas(modifier = modifier) {
            ...
            translate(top = 1.dp.toPx() * (if (isError) droopAngle.value / 3f else 0f)) {
                ...
                withTransform({
                    rotate(
                        degrees = (if (isError) droopAngle.value else 0f) as Float,
                        pivot = center.copy(y = stemHeight)
                    )
                }) {
